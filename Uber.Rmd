...
Travelling from one place to another is a hard task in big cities. For that reason, some companies like Uber provide their users with cars, so they can easily travel in exchange of some money. Also, users can take the role of the drivers and get payed. This service is specially important for tourists, because they do not own a vehicle in the city and using the public transport may take a long time; but also for people living in the city, who do not own a car, or who do not want to waste time finding a parking place. There are many advantages and disadvantages about services of this kind, but it is not the main objective of this document to discuss them. 

Uber provides some datasets related with the use of their services in different cities around the world. It is possible to use that data in order to create a graph, where city areas are the nodes and their adjacencies are the edges. Next, we can analyze the traffic flow between different parts of a city. By doing that, the objective of this document is to detect congested areas (meaning areas where the Uber traffic flow is very dense). Once that metric is calculated, it would be possible to use different criteria to find out the most congested area. That would mean that it is the area where most cars drive in and out, and so it is an important area. Moreover, some community detection algorithms can also be applied to this scenario. By performing this type of analysis, it is possible to identify groups of congested areas (or, in the same way, non congested areas). Note that there is one assumption made here: the Uber traffic is representative of the general traffic flow, meaning that if a particular area is traversed by many Uber cars, then it is likely that the same happens with ordinary traffic.

Those results could be used to identify which route can be followed to avoid dense traffic, or to identify important parts of the city (assuming that cars tend to drive to areas where most of the people activity is focused).

Once different criteria are used for each kind of annalysis, some discussion will be made regarding how robust or accurate are the used methods, together with some possible future improvements.

The code used during all the process can be found attached at the end of this document.
...
Each area is a polygon, and is identified by an ID. In the case of Amsterdam, there are `r length(polys)` of them, where the IDs are numbers from 0 to `r length(polys) - 1`. Since GeoJSON is a well kown format, there are different tools to read data from the previous file, such as the _readOGR_ function (from _rgdal_ package). The geographical representation of the zones in Amsterdam can be found below. Recall that a zone can be formed by different polygons, or have holes. However, the vast majority of zones are defined by simple polygons, and those cases could barely be seen. Hence, all of them are shown using the same color in the figure below. Note that the city center of Amsterdam is located where most of the small areas are, and they tend to be larger when their distance to the city center increases (even tough there are some exceptions).


```{r echo = FALSE, fig.width=4.25, fig.height=4.25, fig.align='center'}
plot(polys, main = "Zones in Amsterdam", col="#f44336",border="black", lwd=1)
```
...
The analysis performed over these 24 graphs is explained in the following sections. Since there are lots of graphs and multiple algorithms, the results are represented in GIF format attached to this documents, where the evolution of the traffic flow during the day is shown dynamically.

## Centrality measures

Once the graph is created, it is time to perform some calculations with different criteria in order to find out the most important areas of the city. The meaning of _important_ may differ from one approach to another, and will determine the interpretation from the information obtained in the results. In this section, different centrality measures will be applied on the graph. Each one of them will be explained, and the results will be shown with some images. 
...
It can be seen in the figure that the most important areas are the big ones, as expected. The darker colors are present in huge areas close to small ones. That means, large areas close to the city center. This makes sense, because if a large area is surrounded by small ones, then the number of potential neighbors can be higher. Also, there are some small areas in the city center with a dark color, meaning that the number of neighbors is high, and therefore its importance is high as well.

Using this centrality, we get a low number of important areas, which are located in or near the city center, which makes sense. Also, there is another important area in the outskirts of the city. However, except for these few cases, the rest of the areas in the graph have a similar color, meaning a similar importance.  

To sum up, this centrality does not seem to be a good aproach, because it does not take into account the Uber trips. However, the results do not differ too much from the expected ones, being most of the important areas located near the city center.

### Strength degree centrality
This one is a similar measure to the previous one. However, there is a very important difference: now, the importance of a node is determined not by its number of edges, but by the weighted sum of its edges. Recall that the weight of an edge between two nodes is the number of Uber cars which drove for a given time between both nodes, assuming that the route is the shortest path between origin and destination.

This centrality seems to be a very good one for this scenario. As said in the introduction of this document, the analysis is performed in order to find out congested (or not congested) areas. Since the importance of a node is directly proportional to the number of cars which traverse that zone, then this one seems to be a good approach.
...
In a weighted graph like this one, the betweenness centrality of a node _A_ measures how many paths between all pairs of other nodes traverse the node A, where that paths are the ones which minimize the sum of weights of the route between origin and destination. In other words, if a node is rated high by this centrality, it means that there are many traffic-free routes which traverse that node. Hence, that node is important for drivers who want to avoid traffic congestions.
...
Again, there are differences between day and night times, even though those are slightly ones. As said before, there is less traffic during the night, so there will be more important nodes by this metric at night. On the contrary, there are less important nodes during the day, because the traffic is higher and so the sum weight of the path also increases. Overall, there is a most important node highlighted all times. That one is a big area close to the city center.

To sum up, high rated nodes by betweenness centrality are important nodes for those drivers who want to avoid traffic, and one node is highlighted over the others.

### Eigenvector centrality
Eigenvector centrality (or just eigen centrality) measures the influence of a node in the network. It is similar to the Page Rank centrality, which will be analyzed later (actually, Page Rank is a variation of eigen centrality). This measure rates higher those nodes who are connected with high rated neighbors. Since this is a weighted graph, the importance of a node is proportional to the sum of weights of their edges, and their neighbors'. In other words, a node becomes more important if the traffic is dense both in that area and in the adjacent ones. 

Note that this is a recursive definition: first node's neighbors are rated high by eigen centrality if the traffic is dense both in them and in their neighbors. Let A -> B -> C be a graph where node A is connected to node B, and B is connected to node C. If the traffic in C is dense, then the importance of B increases, which means that the importance of A also increases. Hence, it can be seen that all nodes indirectly connected to other nodes affect their eigen centrality.

This measure fits for our scenario, because it is expected that if the traffic is dense in an area, then it will also be dense in close areas. First, because people tend to drive to that geographical places. Second, because if an area is congested, then it is expected that people take alternative routes avoiding that area, meaning that they would drive though close areas (neighbor nodes in the graph). Hence, it is clear that an important node has an influence on their neighbors.

![Eigenvector centrality in Amsterdam at 18:00h](img/eigen.cent/19.png){width=300px} ![Eigenvector centrality in Amsterdam at 3:00h](img/eigen.cent/04.png){width=300px}

In this case, the most congested areas are very different depending on whether it is day or night time. As shown in the images, eigen centrality for the areas in Amsterdam at 18:00h rates high those nodes located in the Northwest of the city center. This is true for most of the day (see the correspondent GIF attached to this document). However, between 2:00h and 6:00h, this is completely different. The most congested areas are those located just in the city center.

While the amount of cars can vary depending on the time, the expected result would be that the most congested areas would remain more or less the same for all the day, even though the traffic would be denser during day time. However, in this case there is a great shift in the importance of nodes depending on the already discussed periods of time. That change happens between 1:00 and 2:00h, and no transition phase is appreciated. However, the important areas are close in both cases, so this is not a radical difference.

Also, note that the important nodes tend to be close from each other for a given time. This is an expected result, because the importance of a node depends on the importance of their neighbors. Hence, it is expected to find that an important node is surrounded by important neighbors.

This centrality is a very good one for our scenario. Together with the strength degree centrality, eigenvector centrality is a good tool to look for congested areas.

### Page Rank centrality
The last analyzed centrality is Page Rank. As said before, Page Rank is a variation of the eigenvector centrality, so it is expected that the results here would be similar to the previous ones. However, Page Rank is originaly designed for directed graphs. Since we are working with an undirected graph, then the results can be not as accurate as expected.

In the case of undirected and weighted graphs, Page Rank is supposed to behave similarly as the strength degree distribution, discussed previously in this document. The only difference is that neighbors' rates have a small effect on a given node. Recall that attached to this document there are GIFs which make the comparation easier, but here some images will be shown comparing both centralities.

![Page Rank centrality in Amsterdam at 16:00h](img/page.rank.cent/17.png){width=300px} ![Strength degree centrality in Amsterdam at 16:00h](img/degree.str.cent/17.png){width=300px}

As seen in the figures, it is true that both centralities behave very similarly. The differences are very small ones, so no special discussion will be made here. The interpretation of the results for Page Rank in this case (an undirected, weighted graph) is the same one as for the strength degree centrality: the congested areas are those which get a high rate by this metric. 
...
![Betweenness comunnities from 9:00h to 10:00h](img/betweenness.com/10.png){width=300px}

Note that this measure is not a good aproach for our scenario. In this case, an edge with a high weight sepparates communities. However, in the real scenario that would mean that both areas are congested, and so they should belong to the same community. Hence, this community detection algorithm does not seem appropriate.

### Fast-Greedy

Fast-Greedy is an iterative algorithm which is based on optimizing the value of a quality function called modularity. The algorithm begins considering each node as an independent community. Subsequently, the communities are merged in such a way that this fusion is locally optimal, i.e. that the merge causes the greatest increase of all in the modularity. It is completed once this value can not be increased further.
...
![Fast-greedy comunnities from 9:00h to 10:00h](img/fast.greedy.com/10.png){width=300px}

### Label Propagation

This one is a very fast and simple algorithm. Label propagation is based on assigning one label out of $k$ in the initial configuration. Then, iteratively each node takes the most frequent label of all its neighbours in a synchronous way until each node has one of the most frequent labels in its neighbourhood.
...
![Label propagation comunnities from 9:00h to 10:00h](img/label.prop.com/10.png){width=300px}
...
![Walktrap comunnities from 9:00h to 10:00h](img/walktrap.com/10.png){width=300px}

# Conclusion and future work

The analysis of the traffic flow in a city can be very important to detect areas which are usually more congested, the time at which this occurs and for how long. Being able to estimate these three variables with precision would be very useful to carry out better urban planning. With this project we intend to make an approximation to this analysis using the traffic flow of Uber users as data.

Although the obtained results allow us to get an idea about how traffic in the city behaves by zones, the truth is that there are a number of restrictions that make it very difficult to perform a more robust analysis. First of all, Uber offers information about the origin and destination areas of the trips, but not of the intermediate zones that it crosses. Hence, trying to infer the route using the shortest path technique does not represent reality. The map of the city only includes zones, which cover several roads. That means that the shortest path in the graph may not be the same as the real route of the trip. That causes a great inaccuracy to perform a more precise analysis.

Moreover, in the graph all the nodes are treated as equal, because the sizes and distances are not taken into account. However, areas on the map vary greatly in size, and distances are important for a driver. Suppose, for example, a case in which there are 5 areas adjacent to each other: 4 small zones and a relatively large area. If the shortest path between two of the small zones is through the other two small zones on the map, the graph will consider that the shortest route will be through the large zone, since the number of jumps would be 2 instead of 3 and. Therefore, an error would be made.

Indeed, the main problem of the presented method is the calculation of shortest paths in the graph. As said before, the route may not be the same as the real one. However, some good points about it is that it is a simple and fast method, which do not require great computing resources. An altertative for this method would be to use some APIs for calculating trip routes, such as Google Maps API or OSRM (Open Source Routing Machine). Using those, the calculated route would be much more precise, and it would be necessary to analyze which zones it traverses. However, those APIs have some restrictions, e.g. number of requests per minute. That was the main reason why those were not used in this analysis.

Despite these drawbacks and the improvements that could be made in the analysis, the results presented in this project may coincide with reality and are valid for knowing the behavior of a city at the zone level, not only of the city of Amsterdam but of any other city that Uber offers in its datasets repository.
...
for (hour in 1:24){
  ...
  for(i in 1:nrow(current_data)) {
    trip <- current_data[i,]
    #Zone ID = position in matrix
    from <- trip$sourceid
    to <- trip$dstid
    
    #Path A->B->C becomes vector (A, B, B, C)
    route <- as.vector(sp[[from]]$vpath[[to]])
    EP = rep(route, each=2)[-1]
    EP = EP[-length(EP)]
    
    #Take edges IDs for accesing E(g[[hour]]))
    edge_ids <- get.edge.ids(g[[hour]], EP)
    
    #Increase weight of edges with previous IDs
